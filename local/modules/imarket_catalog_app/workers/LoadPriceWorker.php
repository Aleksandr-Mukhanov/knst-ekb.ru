...
class LoadPriceWorker {
    ...
    public function __construct () {
        $this->Logger = new ImarketLogger("/upload/log/LoadPriceWorker/");
       ...
       // if (!empty($this->arCompletedTasks)) {
           ...
           // foreach ($this->arCompletedTasks as $taskId => $arTask) {
              ...
              //  $this->GetExportAttributes(1); // TODO можно получить для одного профиля, они для всех одинаковые,
                ...
                $this->currentProfileId = 1;// $this->arOurPricingProfiles[$taskId];
                $this->Logger->log("LOG", "Получаем данные для профиля ".$this->currentProfileId);
                $arProfileData = $this->restAPI->GetProfilePrices([$this->currentProfileId => $this->arOurPricingProfiles[$this->currentProfileId]]);
                print_r("Всего цен: ".count($arProfileData).PHP_EOL);
                file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/log/LoadPriceWorker/debugData_".$this->currentProfileId.".json", json_encode($arProfileData,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT));
...
//            $this->UpdateDeliveryData();
    ...
    private function GetModes() {
        ...
        file_put_contents($_SERVER["DOCUMENT_ROOT"]."/upload/log/LoadPriceWorker/models_".$this->currentProfileId.".json", json_encode($this->arCatalogAppModels,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT));
    ...
    private function GetVendors() {
        ...
        //print_r($this->arCatalogAppVendors);
        print_r(PHP_EOL."Всего получено брендов ".count($this->arCatalogAppVendors).PHP_EOL);
...
//                $actionIdList[$actionId] = $actionId;
    ...
    private function GetDiscountProducts() {
        ...
        print_r("Получено, всего товаров со скидкой: ".count($this->discountProducts).PHP_EOL);
    ...
    private function GetCatalogGoods() {
        ...
        print_r("Получено, всего товаров: ".count($this->arCatalogDataByXML_ID).PHP_EOL);
...
(new LoadPriceWorker())->StartWorker();
