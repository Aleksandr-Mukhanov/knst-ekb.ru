...
Class imarket_catalog_app extends CModule {
    const MODULE_ID = 'imarket_catalog_app';
    var $MODULE_ID = 'imarket_catalog_app';
    var $MODULE_VERSION;
    ...
    function __construct () {
        ...
        $this->modulePath = $_SERVER["DOCUMENT_ROOT"] . "/local/modules/" . self::MODULE_ID;
        $this->modulePlace = 'local/modules';
    }
        ...
        if (is_dir($p = $this->modulePath . '/admin')) {
            if ($dir = opendir($p)) {
                while (false !== $item = readdir($dir)) {
                    if ($item == '..' || $item == '.' || $item == 'menu.php' || $item == 'img')
                        continue;
              ...
              /*  if (isset($_REQUEST["create_worker"])) {
                ...
                if (isset($_REQUEST["update_worker"])) {
                    CAgent::AddAgent(
                        ...
                        60*60*24,                                 // интервал запуска - 1 сутки
                        "",                                  // дата первой проверки на запуск
                        "Y",                                 // агент активен
                        "",                                  // дата первого запуска
                        500);                                // сортировка
...
*/
                ...
                if($_REQUEST["catalogApp_user"] && $_REQUEST["catalogApp_pass"]) {
