										...
										<div class="properties__container properties props_list">
											<? foreach ($arResult["SKU_PROPERTIES"] as $key => $arProp) { ?>
												<? if (!$arProp["IS_EMPTY"] && $key != 'ARTICLE') : ?>
													<? $bHasValue = ($arResult["TMP_OFFERS_PROP"][$arProp["CODE"]]
		...
		<div class="flexbox flexbox--row">
					...
					<div class="product-detail-gallery__thmb-container text-center">
						<div class="product-detail-gallery__thmb-inner<?= ($bVertical ? ' vertical' : ''); ?>">
							<? if ($countPhoto > 1 || $showCustomOffer) : ?>
								<div class="product-detail-gallery__slider owl-carousel owl-theme thmb<?= ($bVertical ? ' product-detail-gallery__slider--vertical' : ''); ?><?= ($countPhoto > 3 ? ' m-photo' : ''); ?>" data-size="<?= $countPhoto; ?>" data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider.big", "dots": false, "autoWidth": true, "margin": 10<? //if($bVertical):
			...
			<div class="product-main">
				...
				<div class="flexbox flexbox--row flex-wrap align-items-normal <?= !$bShowPropsBlock ? 'justify-center' : '' ?>">
					<? if ($bShowPropsBlock) : ?>
						<div class="product-chars flex-50">
							...
							<? if (($arResult['DISPLAY_PROPERTIES'] || $arResult['OFFER_PROP']) && $arParams['VISIBLE_PROP_COUNT'] > 0) : ?>
								<div class="char-side">
									<div class="char-side__title font_slg darken"><?= ($arParams["T_CHARACTERISTICS"] ? $arParams["T_CHARACTERISTICS"] : Loc::getMessage("T_CHARACTERISTICS")); ?></div>
									<div class="properties list">
										<ul class="feature <?= !$bShowMoreLink ? 'js-offers-prop' : '' ?>">
											<? $j = 0; ?>
									...
									<div class="more-char-link to-description"><span class="choise colored_theme_text_with_hover font_sxs dotted" data-block=".js-scrolled"><?= Loc::getMessage('ALL_CHARS'); ?></span></div>
								</div>
							<? endif; ?>
					...
					?>
					<div class="product-action flex-49">
						<div class="info_item">
							<div class="middle-info-wrapper main_item_wrapper">
								<div class="shadowed-block product-price-calculator">
									<div class="full-price-block">
										...
										<?*/ ?>
											...
											<div class="prices_block">
												...
												<div class="cost prices detail">
													<span class="font_mxs"><b>Цена:</b></span>
													<? if ($arResult["OFFERS"]) : ?>
												...
												<span class="sum-price"><?= GetMessage("TITLE_TOTAL_PRICE") ?><span id="total-price"><?= GetMessage("TITLE_TOTAL_COUNT") ?></span><?= GetMessage("CURRENCY_TOTAL_COUNT") ?></span>
												...
												?>
												<div class="quantity_block_wrapper">
													<? echo '<span class="availability">Наличие:</span>' . $arQuantityData["HTML"]; ?>
										...
										<div class="price-modal">
											<a href="#" data-event="jqm" data-param-form_id="PRICE_EXCEL" data-name="PRICE_EXCEL" onclick="goModalCustom()"><?= CMax::showIconSvg("excel", SITE_TEMPLATE_PATH . "/images/svg/catalog/excel.svg"); ?> Скачать прайс Excel</a>
											<a href="#" data-event="jqm" data-param-form_id="PRICE_CALC" data-name="PRICE_CALC" onclick="goModalCustom()"><?= CMax::showIconSvg("price_small", SITE_TEMPLATE_PATH . "/images/svg/catalog/price_small.svg"); ?> Рассчитать мой список</a>
											<span class="prompt" data-tooltip="Отправьте нам ваш список, а мы все подсчитаем"><?= CMax::showIconSvg("ask_product", SITE_TEMPLATE_PATH . "/images/svg/catalog/ask_product.svg"); ?></span>
										</div>
									</div>
								</div>
...
<? $this->EndViewTarget(); ?>
<script>
	//Калькулятор расчета стоимости РВД с фитингами
	function productPriceCalculator() {

		var startProductPrice = $(".product-price-calculator .price_value").first().text();
		var quanityProductVal = $("input[type=text][name=quantity]").val();
		var quanityProduct = parseInt(quanityProductVal);
		
		if (quanityProduct > 0) {
			var totalProductPrice = startProductPrice * quanityProduct;
		}

		$("#total-price").text(totalProductPrice);
	};
	// end stickerPriceCalculator

	$(document).ready(productPriceCalculator);

	$('.product-price-calculator').change(productPriceCalculator);
</script>
