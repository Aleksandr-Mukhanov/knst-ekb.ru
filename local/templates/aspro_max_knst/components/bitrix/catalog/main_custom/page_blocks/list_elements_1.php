					...
					<?if($posSectionDescr=="BOTH"):?>
						<?if ($arSection[$section_pos_top]):?>
							<div class="group_description_block top muted777">
								<div><?=$arSection[$section_pos_top]?><button onclick="readMoreSeo()" id="myBtn" class="btn btn-default btn-lg has-ripple">Читать далее</button></div>
							</div>
						<?endif;?>
					<?elseif($posSectionDescr=="TOP"):?>
						<?if ($arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]):?>
							<div class="group_description_block top muted777">
								<div><?=$arSection[$arParams["SECTION_PREVIEW_PROPERTY"]]?><button onclick="readMoreSeo()" id="myBtn" class="btn btn-default btn-lg has-ripple">Читать далее</button></div>
							</div>
						<?elseif ($arSection["DESCRIPTION"]):?>
							<div class="group_description_block top muted777">
								<div><?=$arSection["DESCRIPTION"]?><button onclick="readMoreSeo()" id="myBtn" class="btn btn-default btn-lg has-ripple">Читать далее</button></div>
							</div>
						<?elseif($arSection["UF_SECTION_DESCR"]):?>
							<div class="group_description_block top muted777">
								<div><?=$arSection["UF_SECTION_DESCR"]?><button onclick="readMoreSeo()" id="myBtn" class="btn btn-default btn-lg has-ripple">Читать далее</button></div>
							</div>
						<?endif;?>
					<?endif;?>
					<?
