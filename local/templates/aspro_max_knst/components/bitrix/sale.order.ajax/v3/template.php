...
?>
	<form action="<?= POST_FORM_ACTION_URI ?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order-main" class="row orderform--v2 bx-<?= $arParams['TEMPLATE_THEME'] ?>" style="opacity: 0">
			<!--	MAIN BLOCK	-->
			<div class="col-md-9 bx-soa form-order-custom">
			...
			<div id="bx-soa-total" class="bx-soa-sidebar">
				...
				<div class="price-in-basket">
					<?/*?>
					<a href="#" data-event="jqm" data-param-form_id="TABLE_EXCEL_BASKET" data-name="TABLE_EXCEL_BASKET" onclick="goModalCustom()"><?= CMax::showIconSvg("excel_icon_big", SITE_TEMPLATE_PATH . "/images/svg/excel_icon_big.svg"); ?> Сформировать таблицу Excel</a><?*/?>
					<a href="/include/basket/download-excel.php"><?= CMax::showIconSvg("excel_icon_big", SITE_TEMPLATE_PATH . "/images/svg/excel_icon_big.svg"); ?> Сформировать таблицу Excel</a>
					<span class="prompt" data-tooltip="Сохраните содержимое корзины в таблицу Excel"><?= CMax::showIconSvg("ask_product", SITE_TEMPLATE_PATH . "/images/svg/catalog/ask_product.svg"); ?></span>
					<a href="/include/basket/download-pdf.php"><?= CMax::showIconSvg("excel", SITE_TEMPLATE_PATH . "/images/svg/printer_icon_big.svg"); ?> Распечатать список</a>
				</div>
				<div class="min-price-error">
					<div class="min-price-error_text">
						<span class="min-price-error_title">Минимальная сумма заказа 5000 руб.</span>
						<span class="min-price-error_descrip">Добавьте еще товаров, <a href="/catalog/">перейти в каталог</a></span>
					</div>
				</div>
			</div>
		</div>
